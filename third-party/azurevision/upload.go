...
func (vision *Vision) AnalyzeImage(imageBytes []byte, order VisualFeatures) (VisionResult, error) {
	query, err := order.String()
	if err != nil {
		return VisionResult{}, err
	}
	apiURL := vision.Endpoint + "/analyze?visualFeatures=" + query

	req, err := http.NewRequest("POST", apiURL, bytes.NewReader(imageBytes))
	if err != nil {
		return VisionResult{}, err
	}
